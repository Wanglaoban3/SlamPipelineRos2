...
// ROS2 port of loop_detector.hpp (ros::NodeHandle -> ParamNodeHandle, chrono timing)
...
public:
   ...
   * @param pnh
   */
  explicit LoopDetector(ParamNodeHandle& pnh) {
...
private:
  ...
  Loop::Ptr matching(const std::vector<KeyFrame::Ptr>& candidate_keyframes, const KeyFrame::Ptr& new_keyframe, hdl_graph_slam::GraphSLAM& graph_slam) {
    ...
    std::cout << "relpose: " << relative_pose.block<3, 1>(0, 3).transpose() << " - " << Eigen::Quaternionf(relative_pose.block<3, 3>(0, 0)).coeffs().transpose() << std::endl;
