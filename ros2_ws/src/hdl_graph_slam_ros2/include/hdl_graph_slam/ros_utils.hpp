...
static Eigen::Isometry3d transform2isometry(const geometry_msgs::msg::TransformStamped& trans) {
  Eigen::Isometry3d mat = Eigen::Isometry3d::Identity();
  const auto& t = trans.transform.translation;
  const auto& r = trans.transform.rotation;
  mat.translation() = Eigen::Vector3d(t.x, t.y, t.z);
  mat.linear() = Eigen::Quaterniond(r.w, r.x, r.y, r.z).toRotationMatrix();
  return mat;
}
...
static geometry_msgs::msg::Transform isometry2transform(const Eigen::Isometry3d& mat) {
  Eigen::Quaterniond quat(mat.linear());
  Eigen::Vector3d trans = mat.translation();
